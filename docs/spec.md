...
| ADR-004 | 语音 = Realtime 端到端 | 实测首包 473ms，且服务端包办 VAD 与打断（成本劣势已知悉并接受） |
...
| 项 | 单价 | 备注 |
| --- | --- | --- |
| Nano Banana 2 出图 · 2K | ~100 积分 | 已定档位 |
| Seedance 2.5 · 6s · **1080p** | **4,740 积分** | 已定档位 |
| Seedance 2.5 · 6s · 720p | 2,640 积分 | 仅作参考，不采用 |

**已定参数：视频 1080p，定妆图 2K。**

账户余额 **320,423 积分**。一套 8 状态素材（1080p）约 **37,920 积分**，
理论上够 8 套——但生成式素材的试错率很高（首尾姿态不闭合、表情跑偏、手部畸变都要重出），
**按实际可用 3–4 套服装规划**。
...
- [ ] **先验证 RISK-1**：生成一段测试视频，`ffprobe` 确认 `.mov` 是否带 alpha
...
1. ~~语音走 Realtime 还是拼装？~~ **已确认（2026-08-19）**：在知悉成本劣势
   （每月多约 ¥120）后维持 Realtime，换取 473ms 首包延迟与省下的 5–8 人天工程。
2. **人格设定的尺度。** 本地模型选的是 uncensored 版本，云端 Qwen 有内容审核。
   两者的行为边界会不一致，需要明确期望。
3. **形象的服装套数。** 每套 1080p 约 37,920 积分，扣除试错实际可做 3–4 套。
