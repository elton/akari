...
**当时的决定**：一张 5K PNG 就有 16MB，三个分辨率合计 35MB，
和这个仓库不收 `*.mov` 是同一条理由，所以壁纸留在本地不入库。

**⚠️ 这个决定是错的，由外部代码审查（Codex）指出。**

`*.mov` 那条规则针对的是**可再生成、每次取材都不同**的视频素材。
壁纸不是：它是一次性的、不可再生成的品牌资产，和已经入库的 `AppIcon.icns` 同一性质。
而且失败是静默的 —— `Makefile` 的拷贝步骤由 `if -d` 守着，
一份没有壁纸的检出会打出一个「声称提供配套壁纸、实际找不到素材」的 app，不报任何错。

**「体积」这个前提本身也不成立**：同一张图转 HEIC 后是 **0.91 MB**（PNG 的 5%，PSNR 46 dB
视觉无损），比 `AppIcon.icns` 的 1.5 MB 还小。质量档取 85 是拐点 ——
70 只省 0.27 MB 却掉 0.37 dB，95 要大 4.3 倍才多 1.28 dB。

**现在**：`assets/brand/wallpaper/akari-wallpaper-5120x2880.heic` 随代码交付；
PNG 母版仍不入库（要别的尺寸从 HEIC 缩，要重制原图见 `assets/brand/README.md` 的提示词）。
只入库这一张 —— `WallpaperCatalog.rank` 按「像素数最接近母版」排序，
其余尺寸在任何显示器上都轮不到，留着只会让人以为它们有用。

### `.gitignore` 排除目录会让否定规则静默失效

修上面那条时踩到的，值得单独记：排除 PNG 母版**必须**写
`assets/brand/wallpaper/*.png`，**不能**写 `assets/brand/wallpaper/`。

排除一个**目录**会让目录内所有 `!` 否定规则失效 —— git 不会递归进被排除的目录，
否定规则根本没有机会匹配。而且它**不报任何错**：
`!assets/brand/wallpaper/*.heic` 明明写着，看起来完全正确，文件就是进不去仓库。
第一次修复因此白做了一轮，`git ls-files` 才是唯一可信的验收手段。
